...
public class ShardPacBioSubReadsUBamByZMWClusterSpark extends GATKSparkTool {
    ...
    protected void runTool( final JavaSparkContext ctx ) {
        ...
        Integer maxZMWnum = getUnfilteredReads().map(read -> read.getAttributeAsInteger(ZMW_ATTRIBUTE_KEY)).max(Comparator.naturalOrder());
        ...
        if (! debugMode) {
            ...
            final JavaPairRDD<Integer, Iterable<GATKRead>> cache = getUnfilteredReads()
                    .groupBy(read -> read.getAttributeAsInteger(ZMW_ATTRIBUTE_KEY) / local_shard_size).cache(); // shard index
